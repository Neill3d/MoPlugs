...
{
	VectorSub(v2, v1, middle);
	VectorMult(middle, 0.5);
	VectorAdd(middle, v1, middle);
}
...
{
	FBVector3d v(v2[0]-v1[0], v2[1]-v1[1], v2[2]-v1[2]);
...
{
	res[0] = (v2[0] - v1[0]) * t + v1[0];
	res[1] = (v2[1] - v1[1]) * t + v1[1];
	res[2] = (v2[2] - v1[2]) * t + v1[2];
}
...
void ClosestPointOnLine2(FBVector3d &pointOnLine, const FBVector3d &A, const FBVector3d &B, const FBVector3d &P, double *t)
{
	FBVector3d tempVector1, tempVector2, diff;

	VectorSub(P, A, tempVector1);
	VectorSub(B, A, tempVector2);

	double distance = VectorLength(tempVector2);
	double dotProduct = DotProduct(tempVector2, tempVector1);
	...
	{
		VectorMult(tempVector2, dotProduct);
		VectorAdd(A, tempVector2, pointOnLine);
	}
}
...
bool FindClosestPointOnCurve( FBVector4d &pointOnCurve, double &curvePercent, FBModelPath3D *pCurve, const FBVector4d &pos, const int numberOfSubdivisions, const int segmentSubdivisions )
{
	...
	if (pCurve->PathKeyGetCount() < 2)
		return false;

	FBVector4d pA, pB;

	pA = pCurve->Total_GlobalPathEvaluate(0.0);

	double minDist = std::numeric_limits<double>::max();
	FBVector3d minP(0.0, 0.0, 0.0);
	int	minSubdivIndex = -1;
	double minPercent = 0.0;

	double step = 100.0 * 1.0 / numberOfSubdivisions;
	double percent = step;
	double lineT = 0.0;

	for (int i=0; i<numberOfSubdivisions; ++i, percent+=step)
	{
		pB = pCurve->Total_GlobalPathEvaluate(percent);

		FBVector3d p;
		
		ClosestPointOnLine2(p, FBVector3d(pA), FBVector3d(pB), FBVector3d(pos) );

		FBVector3d diff;
		VectorSub( p, FBVector3d(pos), diff );
		double len = VectorLength( diff );

		if (len < minDist)
		{
			VectorSub( FBVector3d(pA), FBVector3d(pB), diff );
			double segmentLength = VectorLength(diff);

			VectorSub( FBVector3d(pA), p, diff );
			double tLength = VectorLength(diff);

			if (segmentLength > 0.0)
				lineT = tLength / segmentLength;
			else lineT = 1.0;

			minDist = len;
			minP = p;
			minSubdivIndex = i;
			minPercent = percent - step * (1.0 - lineT);
		}

		pA = pB;
	}

	// now divide line and get more precise value
	if (minSubdivIndex >= 0)
	{
		double lineStep = step / segmentSubdivisions;
		double percent = step * minSubdivIndex;

		pA = pCurve->Total_GlobalPathEvaluate(percent);
		percent += lineStep;

		for (int i=0; i<segmentSubdivisions; ++i, percent += lineStep )
		{
			pB = pCurve->Total_GlobalPathEvaluate(percent);
			...
			if (len < minDist)
			{
				...
				minDist = len;
				minP = p;
				minPercent = percent - lineStep * (1.0 - lineT);
			}

			pA = pB;
		}

		pointOnCurve = FBVector4d(minP[0], minP[1], minP[2], 1.0);
		curvePercent = minPercent;
	}

	return (minSubdivIndex >= 0);
}
...
bool PlaneIntersection( const FBVector4d &plane1, const FBVector4d &plane2, FBVector3d &start, FBVector3d &dir )
{
	...
	VectorAdd( normal, normal2, start );
	return true;
}
...
#endif
	frustum.CalculateFrustum( proj, modl );
}
*/
bool ComputeCameraValues(FBRenderOptions *pOptions, FBMatrix &modelview, FBVector3d &cameraPos)
{
	FBCamera* lCamera = pOptions->GetRenderingCamera();
	if ( FBIS(lCamera, FBCameraSwitcher) )
		lCamera = ((FBCameraSwitcher*) lCamera)->CurrentCamera;

	if (lCamera == nullptr) return false;
	
	lCamera->GetVector( cameraPos );
#ifdef ORSDK2013
	modelview = lCamera->GetMatrix(kFBModelView);
#else
	lCamera->GetCameraMatrix( modelview, kFBModelView );
#endif
	return true;
}
...
bool ComputeCameraValues(FBCamera *pCamera, FBMatrix &modelview, FBMatrix &projection, FBMatrix &viewIT, FBVector3d &cameraPos, int &width, int &height, double &farPlane)
{
...
#ifdef ORSDK2013
	projection = pCamera->GetMatrix(kFBProjection);
	modelview = pCamera->GetMatrix(kFBModelView);
#else
	pCamera->GetCameraMatrix( projection, kFBProjection );
	...
	farPlane = pCamera->FarPlaneDistance;
	
	width = pCamera->CameraViewportWidth;
	height = pCamera->CameraViewportHeight;
...
bool ComputeCameraValues(FBRenderOptions *pOptions, FBMatrix &modelview, FBMatrix &projection, FBMatrix &viewIT, FBVector3d &cameraPos, int &width, int &height, double &farPlane)
{
	...
	ComputeCameraValues( lCamera, modelview, projection, viewIT, cameraPos, width, height, farPlane );
...
void ComputeCameraRay( FBMatrix &modelview, FBVector3d &outRayDir )
{
	...
	outRayDir[0] = mViewUp[0];
	outRayDir[1] = mViewUp[1];
	outRayDir[2] = mViewUp[2];
}
