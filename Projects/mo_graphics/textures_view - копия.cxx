...
//! a constructor
CGUITexturesModel::CGUITexturesModel()
	: CGUIWidgetModel()
{
...
void CGUITexturesModel::AssignTexturesManager( CGPUTexturesManager *manager )
{
	mTexturesManager = manager;
}

void CGUITexturesModel::AssignContextMenu(FBGenericMenu *pMenu)
{
	mMenu = pMenu;
}

const int CGUITexturesModel::GetNumberOfTextures()
{
	return mNumberOfComponents;
}

const double CGUITexturesModel::GetPan()
{
	return mPan;
}

CGUILayout *CGUITexturesModel::CreateLayout()
{
...
void CGUITexturesModel::OnMouseUp(const int pMouseX, const int pMouseY, const int pButtonKey, const bool hasMotion)
{
...
bool CGUITexturesModel::UpdateFromTexturesManager()
{
	if (mTexturesManager == nullptr)
		return false;
	...
	if (numCachedTextures > 0)
	{
		...
		mNumberOfComponents = numCachedTextures;
		CGUITexturesComponent *pComponents = new CGUITexturesComponent[numCachedTextures];
		mComponents.reset( (CGUIComponent*) pComponents );

		double width, height;

		CGUITexturesComponent *comp = pComponents;
		for (int i=0; i<numCachedTextures; ++i)
		{
			...
			comp->Set(0.0, 0.0, width, height, mTexturesManager->GetTextureId(i) );
