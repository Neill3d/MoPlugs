 ...
 *	Constructor.
 ************************************************/
ORTextureAdvanceBlend::ORTextureAdvanceBlend( const char* pName, HIObject pObject ) : FBLayeredTexture( pName, pObject )
{
	FBClassInit;
	mSupported = true;
	if (glewFirstRun)
...
bool ORTextureAdvanceBlend::FBCreate()
{
    ...
    FBPropertyPublish(this, CustomComposition, "CustomComposition", NULL, ORTextureAdvanceBlend_CustomCompositionSet);
...
void ORTextureAdvanceBlend::TextureLayerComposition(FBTime pTime,FBTime pTimeInCurrentTimeRef, int pWidth, int pHeight)
{
	...
	else if (mLoaded && Layers.GetCount() == 2)
    {
		...
		ECompositeShader shaderType = ECompositeShader(eCompositeShaderBlendNormal + BlendMode);

		bool lUseMask = UseMask;
		FBTexture *lMask = (Mask.GetCount()>0) ? (FBTexture*)Mask.GetAt(0) : nullptr;

		if (false == mShaderManager.CheckAndLoadShader( shaderType, UseMask && (nullptr!=lMask) ) )
		{
			mLoaded = false;
			return;
		}
		...
		mShaderManager.Bind(shaderType, lUseMask & (lMask!=nullptr));
		float blendColor[4] = {1.0, 1.0, 1.0, 1.0};
		mShaderManager.SetBlendUniforms(Layers[1]->Alpha, false, blendColor); // lUseMask & (lMask!=nullptr));
