...
{
	if (strcmp(pShader->ClassName(), PROJTEX_CLASSNAME) == 0)
	{
		return true;
	}
...
bool CProjectors::Prep(FBShader *pShader)
{
...
bool CProjectors::Bind(const int shaderFragmentId, const int projectorsUniformLoc)
{
	...
	{
		if (mProjectorsModels[i].textureId > 0)
		{
			glActiveTexture(GL_TEXTURE10 + i);
			glBindTexture(GL_TEXTURE_2D, (GLuint) mProjectorsModels[i].textureId );
		}
	}
